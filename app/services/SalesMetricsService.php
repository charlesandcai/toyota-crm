...
class SalesMetricsService
{
    ...
    public function getMonthlyData(int $year, int $month): array
    {
        ...
        $closed = $this->leadModel->getClosedDealsCount($releaseStage, $year, $month);
    ...
    public function getAverageClosingRatio(): float
    {
        ...
        $stmt = $db->prepare(
            "SELECT 
             ...
             FROM leads l
             WHERE l.archived = 0 AND YEAR(l.initial_contact_date) = YEAR(CURDATE()) AND MONTH(l.initial_contact_date) <= MONTH(CURDATE())"
        );
        $stmt->execute();
        $result = $stmt->fetch();
    ...
    public function getMonthlySummary(): array
    {
        ...
        for ($m = 1; $m <= 12; $m++) {
            $leads = $this->leadModel->getLeadsCountByMonth($year, $m);
            $deals = $this->leadModel->getDealsCountByMonth($year, $m);
