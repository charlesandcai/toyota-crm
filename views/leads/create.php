...
<div class="card section-card">
    <div class="card-body">
        <form method="POST" action="<?= Url::route('leads/store') ?>" id="leadForm">
            <?= Security::csrfField() ?>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="lead_name" class="form-control <?= isset($errors['lead_name']) ? 'is-invalid' : '' ?>" 
                           required value="<?= Security::escape($_POST['lead_name'] ?? '') ?>">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="company" class="form-control" value="<?= Security::escape($_POST['company'] ?? '') ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-medium">Phone</label>
                    <input type="tel" name="phone" class="form-control" value="<?= Security::escape($_POST['phone'] ?? '') ?>">
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="location" class="form-control" 
                           value="<?= Security::escape($_POST['location'] ?? '') ?>">
                </div>
                <div class="col-md-6">
                    ...
                    <input type="date" name="birthday" class="form-control <?= isset($errors['birthday']) ? 'is-invalid' : '' ?>" 
                           value="<?= Security::escape($_POST['birthday'] ?? '') ?>">
                ...
                <div class="col-md-4">
                    ...
                    <select name="status_id" class="form-select">
                        ...
                        <?php foreach ($statuses as $s): ?>
                            <option value="<?= $s['id'] ?>" <?= (int)($_POST['status_id'] ?? 0) === (int)$s['id'] ? 'selected' : '' ?>>
                ...
                <div class="col-md-4">
                    ...
                    <select name="opportunity_stage_id" class="form-select">
                        ...
                        <?php foreach ($stages as $s): ?>
                            <option value="<?= $s['id'] ?>" <?= (int)($_POST['opportunity_stage_id'] ?? 0) === (int)$s['id'] ? 'selected' : '' ?>>
                                <?= Security::escape($s['name']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="priority_id" class="form-select">
                        ...
                        <?php foreach ($priorities as $p): ?>
                            <option value="<?= $p['id'] ?>" <?= (int)($_POST['priority_id'] ?? 0) === (int)$p['id'] ? 'selected' : '' ?>>
                ...
                <div class="col-md-4">
                    ...
                    <select name="source_id" class="form-select">
                        ...
                        <?php foreach ($sources as $src): ?>
                            <option value="<?= $src['id'] ?>" <?= (int)($_POST['source_id'] ?? 0) === (int)$src['id'] ? 'selected' : '' ?>>
                ...
                <div class="col-md-4">
                    ...
                    <select name="color_id" class="form-select">
                        ...
                        <?php foreach ($colors as $c): ?>
                            <option value="<?= $c['id'] ?>" <?= (int)($_POST['color_id'] ?? 0) === (int)$c['id'] ? 'selected' : '' ?>>
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="release_date" class="form-control <?= isset($errors['release_date']) ? 'is-invalid' : '' ?>" 
                           value="<?= Security::escape($_POST['release_date'] ?? '') ?>">
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="initial_contact_date" class="form-control <?= isset($errors['initial_contact_date']) ? 'is-invalid' : '' ?>" 
                           value="<?= Security::escape($_POST['initial_contact_date'] ?? date('Y-m-d')) ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-medium">Last Contact Date</label>
                    <input type="date" name="last_contact_date" class="form-control" 
                           value="<?= Security::escape($_POST['last_contact_date'] ?? '') ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-medium">Next Step</label>
                    <input type="text" name="next_step" class="form-control" placeholder="e.g. Send quote" 
                           value="<?= Security::escape($_POST['next_step'] ?? '') ?>">
                </div>
                <div class="col-md-4">
                    ...
                    <input type="date" name="next_step_date" class="form-control <?= isset($errors['next_step_date']) ? 'is-invalid' : '' ?>" 
                           value="<?= Security::escape($_POST['next_step_date'] ?? '') ?>">
                </div>

                <div class="col-12">
                    <label class="form-label fw-medium">Notes</label>
                    <textarea name="notes" class="form-control" rows="3"><?= Security::escape($_POST['notes'] ?? '') ?></textarea>
                </div>
                ...
                <div class="col-12">
                    ...
                    <a href="<?= Url::route('leads') ?>" class="btn btn-outline-secondary ms-2">Cancel</a>
                </div>
            </div>
        </form>
    </div>
